...
def format_number_with_commas(value):
    """
    Converts numeric input into comma-separated format while typing.
    Example:
    4000 -> 4,000
    25000 -> 25,000
    """
    ...
    # Allow decimals while typing
    if "." in value:
        whole, decimal = value.split(".", 1)
        whole = "".join(char for char in whole if char.isdigit())
        decimal = "".join(char for char in decimal if char.isdigit())

        if whole == "":
            whole = "0"

        return f"{int(whole):,}.{decimal[:2]}"
...
def parse_money_value(value):
    """
    Converts formatted money text into float.
    Example:
    4,000 -> 4000.0
    4,000.50 -> 4000.5
    """
...
def format_money_display(value):
    """
    Converts numeric value into peso format with 2 decimals.
    Example:
    4000 -> ₱4,000.00
    """
    try:
...
def limit_text_length(max_length):
    """
    Returns a validation function for Tkinter Entry max length.
    Usage:
...
def apply_money_typing_format(entry_widget, variable):
    """
    Formats a money field while typing.
    Example:
    User types 4000 -> field becomes 4,000
    """

    current_value = variable.get()
    formatted_value = format_number_with_commas(current_value)

    if current_value != formatted_value:
        cursor_position = entry_widget.index(tk.INSERT)

        variable.set(formatted_value)
...
def apply_money_typing_format(entry_widget, variable):
    ...
    if current_value != formatted_value:
        entry_widget_position = entry_widget.index(tk.INSERT)

        variable.set(formatted_value)

        try:
            entry_widget.icursor(min(entry_widget_position + 1, len(formatted_value)))
        except Exception:
            entry_widget.icursor(tk.END)
...
class LoginPage(tk.Frame):
    ...
    def limit_entry_length(self, value):
        return len(value) <= 16
    ...
    def build_login_card(self):
        ...
        vcmd = (self.register(self.limit_entry_length), "%P")
        ...
        self.login_button = tk.Button(
            ...
            disabledforeground="#d8cbb6",
            bd=0,
            cursor="hand2",
            font=("Segoe UI", 13, "bold"),
            height=2
        )
        self.login_button.pack(fill="x", pady=(2, 14))
    ...
    def register_failed_login_attempt(self):
        ...
        self.start_login_lockout()

    def start_login_lockout(self):
        self.is_locked = True
        self.lockout_remaining = LOGIN_LOCKOUT_SECONDS

        if hasattr(self, "login_button"):
            self.login_button.config(
                state="disabled",
                text=f"Locked ({self.lockout_remaining}s)",
                bg="#9b8f72",
                cursor="arrow"
            )
        ...
        if hasattr(self, "login_status_var"):
            self.login_status_var.set(
                f"Too many failed attempts. Please wait {self.lockout_remaining} seconds."
            )

        messagebox.showwarning(
            "Login Temporarily Locked",
            f"Too many failed login attempts.\n\nPlease wait {LOGIN_LOCKOUT_SECONDS} seconds before trying again."
        )
    ...
    def update_lockout_timer(self):
        ...
        if self.lockout_remaining <= 0:
            ...
            if hasattr(self, "login_status_var"):
                self.login_status_var.set("You may try logging in again.")
...
class DashboardPage(ttk.Frame):
    ...
    def build_ui(self):
        ...
        self.scroll_canvas.bind(
            "<Enter>",
            self.bind_dashboard_mousewheel
        )

        self.scroll_canvas.bind(
            "<Leave>",
            self.unbind_dashboard_mousewheel
        )

        self.bind(
            "<Destroy>",
            self.cleanup_dashboard_bindings
        )
...
class PaymentPage(ttk.Frame):
    ...
    def build_ui(self):
        ...
        self.payment_table.heading("client_name", text="Client")
    ...
    def is_current_user_admin(self):
        return str(self.app.current_user.role or "").upper() == "ADMIN"

    def apply_role_permissions_to_payment_actions(self):
    ...
    def verify_selected_payment(self):
        if not self.is_current_user_admin():
            messagebox.showwarning(
                "Permission Denied",
                "Only admin accounts can verify payments."
            )
            return
    ...
    def reject_selected_payment(self):
        if not self.is_current_user_admin():
            messagebox.showwarning(
                "Permission Denied",
                "Only admin accounts can reject payments."
            )
            return
...
class PaymentFormWindow(tk.Toplevel):
    ...
    def build_ui(self):
        ...
        tk.Label(
            summary_box,
            text=self.get_booking_summary_text(),
            bg=APP_BG,
            fg=TEXT_DARK,
            font=("Segoe UI", 10),
            justify="left",
        ...
        payment_method_box = ttk.Combobox(
            ...
            values=("GCash", "Maya", "Bank Transfer", "Cash"),
            state="readonly"
        )
        ...
        self.reference_number_entry = ttk.Entry(
            form,
    ...
    def save_payment(self):
        try:
            ...
            confirm = messagebox.askyesno(
                ...
                f"Amount: {format_money_display(amount)}\n"
                f"Method: {payment_method}\n"
                f"Reference No.: {reference_number or 'N/A'}\n"
                f"Date: {payment_date}\n\n"
                f"{verification_note}"
            )
